...
Domain-size extrapolation, not super-resolution. See README.md.
    ...
    uv run python case/scale/run.py split --check                  # verify splits/scale/<campaign>.yaml
    uv run python case/scale/run.py inference --campaign gdl --model abupt   # GPU stage, one cell (+ its GIFs)
    uv run python case/scale/run.py render --cell configs/scale/gdl/abupt_all.yaml   # only the GIFs (GPU)
    uv run python case/scale/run.py metric --cell configs/scale/gdl/abupt_all.yaml --workers 16   # CPU stage
    uv run python case/scale/run.py report                         # curves + report.md per campaign
    uv run python case/scale/run.py all                            # inference, metric, report over every cell

Every cell is a config under configs/scale/<campaign>/<model>_all.yaml (poreml.study): the
...
configs/<campaign>/ files. Outputs: case/scale/<campaign>/<model>_all/ and, per campaign,
case/scale/<campaign>/report.md + curves/.
"""
