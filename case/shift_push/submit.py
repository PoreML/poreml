...
Enumerates configs/shift_push/<campaign>/*.yaml and submits every cell in state `pending`
...
Usage:
  uv run python case/shift_push/submit.py [--dry-run] [--only SUBSTR] [--throttle 8] [--reservation NAME]
  uv run python case/shift_push/submit.py --metric [--throttle 8]   # the CPU-only array for cells whose frames wait
