...
The geometry source is the only thing that changes. See README.md.
    ...
    uv run python case/shift_push/run.py split --check                  # verify splits/shift/<campaign>.yaml
    uv run python case/shift_push/run.py inference --campaign gdl --model abupt   # GPU stage, one cell
    uv run python case/shift_push/run.py metric --cell configs/shift_push/gdl/abupt_gen.yaml --workers 16   # CPU stage
    uv run python case/shift_push/run.py report                         # curves + report.md per campaign
    uv run python case/shift_push/run.py all                            # inference, metric, report over every cell

Every cell is a config under configs/shift_push/<campaign>/<model>_gen.yaml (poreml.study): the
...
configs/<campaign>/ files. Outputs: case/shift_push/<campaign>/<model>_gen/ and, per campaign,
case/shift_push/<campaign>/report.md + curves/.
"""
