"""The Scale-Up study: the `<model>_all` checkpoints (trained on generated + uCT rocks at the
128 class), without retraining, on the campaign's 256-class domains at fixed voxel spacing.
Domain-size extrapolation, not super-resolution. See README.md.

    uv run python case/scale_push/run.py split --check                  # verify splits/scale/<campaign>.yaml
    uv run python case/scale_push/run.py inference --campaign gdl --model abupt   # GPU stage, one cell (+ its GIFs)
    uv run python case/scale_push/run.py inference --cell <cell> --shared        # one of several workers on the cell (no GIFs)
    uv run python case/scale_push/run.py render --cell configs/scale_push/gdl/abupt_all.yaml   # only the GIFs (GPU)
    uv run python case/scale_push/run.py metric --cell configs/scale_push/gdl/abupt_all.yaml --workers 16   # CPU stage
    uv run python case/scale_push/run.py report                         # curves + report.md per campaign
    uv run python case/scale_push/run.py all                            # inference, metric, report over every cell

Every cell is a config under configs/scale_push/<campaign>/<model>_all.yaml (poreml.study): the
...
configs/<campaign>/ files. Outputs: case/scale_push/<campaign>/<model>_all/ and, per campaign,
case/scale_push/<campaign>/report.md + curves/.
"""
...
STAGES = ("split", "inference", "render", "metric", "report")

# Split rule: every finished 256-class run of the campaign, gen and uCT alike — the whole pool,
# no draw, no quota. Size class = the rock's max axis.
SIZE_CLASS = 256
POOL_FAMILIES: dict[str, tuple[str, ...] | None] = {c: None for c in CAMPAIGNS}
...
def stage_split(args) -> None:
    ...
    for campaign in args.campaigns:
        ...
        if args.check:
            ...
            print(f"{path.relative_to(REPO)}: {'matches' if current == test else 'DIFFERS FROM'} the pool ({len(test)} runs)")
            continue
        ...
        header = (
            ...
            f"from {root}.\n"
            f"Every finished run whose rock max axis is {SIZE_CLASS}, families "
            f"{'all' if families is None else ', '.join(families)}: {len(test)} of {len(refs)} discovered.\n"
...
def stage_inference(args) -> None:
    """GPU stage: store the keyframes of every 64-step window of every run; nothing scored. Then render."""
    ...
    for cell, cfg, ckpt in scored(args):
        ...
        if state in ("finished", "metric") and not args.force:
            ...
            try:
                payload = inference_split(
                    cfg, ckpt=ckpt, split="test", out_dir=cell.config.out_dir, stride=stride, shared=args.shared
                )
            ...
            if not payload["complete"]:
                print(f"  {payload['n_missing']} window(s) still held by other workers; the last worker writes the sentinel")
        if args.shared:  # the GIFs are one worker's job, not every worker's: submit.py --render once the cell is complete
            continue
        _render(cell, cfg, ckpt, args)


def stage_render(args) -> None:
    """GPU stage on its own: the truth | prediction GIFs of a cell whose inference already ran (or not)."""
    for cell, cfg, ckpt in scored(args):
        _render(cell, cfg, ckpt, args)


def _render(cell, cfg, ckpt, args) -> None:
    """`scheme.render` runs of the cell, one per rock family, as 64-step truth | prediction GIFs under
    <out_dir>/render/<run_id>/ — training's periodic render, a quarter into the run. Existing GIFs are
    kept unless --force; a failed render is reported and skipped, never a failed cell."""
    from poreml.study import render_cell

    n = cell.scheme.render
    if n == 0:
        return
    print(f"-- render {cell.config.name}: {n} run(s), horizon {cfg.rollout.horizon}")
    gifs = render_cell(cfg, ckpt=ckpt, out_dir=cell.config.out_dir, n=n, force=args.force)
    print(f"  {len(gifs)} of {n} GIFs present")
...
def main() -> None:
    ...
    ap.add_argument(
        "--shared",
        action="store_true",
        help="inference: one of several workers on the same cell — windows are claimed through the store, a held one "
        "skipped, the sentinel written by whichever worker finds the cell complete; no GIFs (see submit.py --render)",
    )
    ap.add_argument(
        "--force",
        action="store_true",
        help="split: overwrite an existing file; inference/render/metric: redo an existing artifact",
    )
    args = ap.parse_args()
